    ...
    return (
        <>
            <div 
                ...
                <div>
                    <ul className="text-white flex gap-7 font-semibold">
                        {navMenu.map(item => (
                        ...
                        {socialMenu.map(item => (
                            <Link key={item.id} href={item.link} target="__blank">
                                <li className="flex cursor-pointer items-center">
                                    <Image width={25} height={25} src={item.imageUrl} alt="Logo social media" />
                                    {item.title}
                                </li>
                            </Link>
                        ))}
            ...
            <div 
                ...
                {isOpen && (
                    <ul className="text-white flex flex-col gap-4 font-semibold">
                        {navMenu.map(item => (
                            <Link href={item.link} key={item.id}>
                                <li 
                                    className={`cursor-pointer gap-2 flex ${activeLink === item.link ? 'text-[#5de4a1]' : ''}`}>
